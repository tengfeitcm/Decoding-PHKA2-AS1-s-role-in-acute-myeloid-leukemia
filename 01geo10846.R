...
if (!file.exists("./GSE10846")) {dir.create("./GSE10846")}
# gset = getGEO(filename = "./GSE10846/GSE10846_series_matrix.txt.gz",getGPL = F)
gset = getGEO("GSE10846",destdir="./GSE10846",getGPL = F)
## 获取分组信息,点开查阅信息
# pdata=pData(gset)
pdata=pData(gset[[1]])
##获取矩阵并校正
...
probe2gene=idmap(index,destdir = "./GSE10846")
...
clin <- pdata %>% 
  dplyr::select(gsm=geo_accession,age=`Age:ch1`,gender=`Gender:ch1`,
                sample.type=characteristics_ch1.3,
                location=`Tissue:ch1`,
  ...
  dplyr::mutate(stage=str_replace_all(stage,"Clinical info: Stage: ",""),
                ...
                immu.subtype=ifelse(immu.subtype=="Unclassified",NA,immu.subtype),
...
clin10846 <- clin
expr10846 <- exprSet %>% 
  dplyr::select(clin10846$gsm)

save(clin10846,expr10846,file = "01geo10846.RData")
